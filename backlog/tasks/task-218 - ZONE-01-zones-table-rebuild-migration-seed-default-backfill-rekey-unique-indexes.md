...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 Two zones can each hold their own active User CA and their own active Host CA at the same time
- [ ] #2 A second active CA of the same type within one zone is still rejected by the database, so rotation semantics are unchanged
- [ ] #3 The same host FQDN, the same identity subject and the same principal name can each exist once per zone and not twice within a zone
