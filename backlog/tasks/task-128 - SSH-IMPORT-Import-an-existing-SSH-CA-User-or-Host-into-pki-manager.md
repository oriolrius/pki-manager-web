...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 An operator can adopt an existing EC CA private key (imported into KMS or already KMS-resident) plus its OpenSSH public key into ssh_cas without regenerating the key
- [ ] #2 A cert signed with the imported CA validates against the published trust anchor (sshd/known_hosts via @cert-authority or TrustedUserCAKeys)
- [ ] #3 No fleet re-trust is required to adopt the existing CA; import writes an ssh.ca.import audit row
- [ ] #4 Import enforces ECDSA-P256 and the same sensitive/exportable posture as create()
<!-- AC:END -->
