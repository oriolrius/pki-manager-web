...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 A user certificate issued in one zone is rejected by a host in another zone, verified against a real sshd
- [ ] #2 A user certificate issued in the same zone as the host is accepted, so the isolation result is not a false positive
